var Vue = (function (exports) {
  ...
  const HTML_TAGS =
    "html,body,base,head,link,meta,css,title,address,article,aside,footer," +
  ...
  function getDepFromReactive(object, key) {
    var _a;
    return (_a = targetMap.get(object)) === null || _a === void 0
      ? void 0
      : _a.get(key);
  }
  ...
  function baseCreateRenderer(options, createHydrationFns) {
    ...
    // css in order to prevent being inlined by minifiers.
    const patch = (
      n1,
      n2,
      container,
    ...
    const patchElement = (
      ...
      if (patchFlag > 0) {
        ...
        if (patchFlag & 16 /* PatchFlags.FULL_PROPS */) {
          ...
          // css
          // this flag is matched when the element has dynamic css bindings
          if (patchFlag & 4 /* PatchFlags.STYLE */) {
            hostPatchProp(el, "style", oldProps.style, newProps.style, isSVG);
          }
          // props
          // This flag is matched when the element has dynamic prop/attr bindings
          // other than class and css. The keys of dynamic prop/attrs are saved for
          // faster iteration.
  ...
  function patchStyle(el, prev, next) {
    ...
    if (next && !isCssString) {
      ...
      // so we always keep the current `display` value regardless of the `css`
  ...
  function useCssModule(name = "$css") {
  ...
  function validateBrowserExpression(
    ...
    try {
      ...
      context.onError(
        createCompilerError(
          45 /* ErrorCodes.X_INVALID_EXPRESSION */,
          node.loc,
          undefined,
          message
        )
  ...
  function createChildrenCodegenNode(branch, keyIndex, context) {
    ...
    if (needFragmentWrapper) {
      if (children.length === 1 && firstChild.type === 11 /* NodeTypes.FOR */) {
        ...
        if (
          !branch.isTemplateIf &&
          children.filter((c) => c.type !== 3 /* NodeTypes.COMMENT */)
            .length === 1
        ) {
          patchFlag |= 2048 /* PatchFlags.DEV_ROOT_FRAGMENT */;
          patchFlagText += `, ${PatchFlagNames[2048 /* PatchFlags.DEV_ROOT_FRAGMENT */]}`;
      ...
      if (node.children.length > 0) {
        if (vnodeTag === KEEP_ALIVE) {
          ...
          if (node.children.length > 1) {
            context.onError(
              createCompilerError(
                46 /* ErrorCodes.X_KEEP_ALIVE_INVALID_CHILDREN */,
                {
  ...
  function buildProps(
    ...
    const analyzePatchFlag = ({ key, value }) => {
      if (isStaticExp(key)) {
        ...
        // treat the dynamic class and css binding of the component as dynamic props
        if (
          isComponent &&
      ...
      switch (propsExpression.type) {
        case 15 /* NodeTypes.JS_OBJECT_EXPRESSION */:
          ...
          if (!hasDynamicKey) {
            ...
            if (
              styleProp &&
              // the static css is compiled into an object,
              // so use `hasStyleBinding` to ensure that it is a dynamic css binding
              (hasStyleBinding ||
                (styleProp.value.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */ &&
                  styleProp.value.content.trim()[0] === `[`) ||
                // v-bind:css and css both exist,
                // v-bind:css with static literal object
  ...
  // modifiers. We also need to merge static and dynamic class / css attributes.
  // - onXXX handlers / css: merge into array
  ...
  const transformModel$1 = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createCompilerError(
          41 /* ErrorCodes.X_V_MODEL_NO_EXPRESSION */,
          dir.loc
        )
      );
    ...
    // check props
    if (
      bindingType === "props" /* BindingTypes.PROPS */ ||
      bindingType === "props-aliased" /* BindingTypes.PROPS_ALIASED */
    ) {
      context.onError(
        createCompilerError(44 /* ErrorCodes.X_V_MODEL_ON_PROPS */, exp.loc)
      );
      return createTransformProps();
    }
    const maybeRef = !true;
  ...
  function baseCompile(template, options = {}) {
    ...
    /* istanbul ignore if */
    {
      if (options.prefixIdentifiers === true) {
        onError(
          createCompilerError(47 /* ErrorCodes.X_PREFIX_ID_NOT_SUPPORTED */)
        );
      } else if (isModuleMode) {
        onError(
          createCompilerError(48 /* ErrorCodes.X_MODULE_MODE_NOT_SUPPORTED */)
        );
      }
    }
    ...
    if (options.cacheHandlers) {
      onError(
        createCompilerError(49 /* ErrorCodes.X_CACHE_HANDLER_NOT_SUPPORTED */)
      );
    }
    if (options.scopeId && !isModuleMode) {
      onError(
        createCompilerError(50 /* ErrorCodes.X_SCOPE_ID_NOT_SUPPORTED */)
      );
    }
  ...
  // Parse inline CSS strings for static css attributes into an object.
  // This is a NodeTransform since it works on the static `css` attribute and
  ...
  const transformVHtml = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(
          51 /* DOMErrorCodes.X_V_HTML_NO_EXPRESSION */,
          loc
        )
      );
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(
          52 /* DOMErrorCodes.X_V_HTML_WITH_CHILDREN */,
          loc
        )
      );
  ...
  const transformVText = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(
          53 /* DOMErrorCodes.X_V_TEXT_NO_EXPRESSION */,
          loc
        )
      );
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(
          54 /* DOMErrorCodes.X_V_TEXT_WITH_CHILDREN */,
          loc
        )
      );
      node.children.length = 0;
  ...
  const transformModel = (dir, node, context) => {
    ...
    if (dir.arg) {
      context.onError(
        createDOMCompilerError(
          56 /* DOMErrorCodes.X_V_MODEL_ARG_ON_ELEMENT */,
          dir.arg.loc
        )
      );
    }
    function checkDuplicatedValue() {
      ...
      if (value) {
        context.onError(
          createDOMCompilerError(
            58 /* DOMErrorCodes.X_V_MODEL_UNNECESSARY_VALUE */,
            value.loc
          )
        );
      }
    ...
    if (
          ...
          if (type.type === 7 /* NodeTypes.DIRECTIVE */) {
            ...
            switch (type.value.content) {
              ...
              case "file":
                ...
                context.onError(
                  createDOMCompilerError(
                    57 /* DOMErrorCodes.X_V_MODEL_ON_FILE_INPUT_ELEMENT */,
                    dir.loc
                  )
                );
                break;
      ...
      context.onError(
        createDOMCompilerError(
          55 /* DOMErrorCodes.X_V_MODEL_ON_INVALID_ELEMENT */,
          dir.loc
        )
      );
    }
  ...
  const transformShow = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(
          59 /* DOMErrorCodes.X_V_SHOW_NO_EXPRESSION */,
          loc
        )
      );
    }
      ...
      if (component === TRANSITION) {
        return () => {
          ...
          if (hasMultipleChildren(node)) {
            context.onError(
              createDOMCompilerError(
                60 /* DOMErrorCodes.X_TRANSITION_INVALID_CHILDREN */,
                {
                  start: node.children[0].loc.start,
                  end: node.children[node.children.length - 1].loc.end,
                  source: "",
  ...
  const ignoreSideEffectTags = (node, context) => {
    if (
      ...
      context.onError(
        createDOMCompilerError(
          61 /* DOMErrorCodes.X_IGNORED_SIDE_EFFECT_TAG */,
          node.loc
        )
      );
      context.removeNode();
